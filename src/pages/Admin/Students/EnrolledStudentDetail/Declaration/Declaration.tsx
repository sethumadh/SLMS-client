...
import EnrollStudentToCurrentTermToCurrentModal from "@/components/Modal/EnrollStudentToCurrentTermModal"
...
export default function Declaration() {
  ...
  return (
    <div>
      ...
      <div className="mt-6 border-t border-gray-100">
        <dl className="divide-y divide-gray-100">
          <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
            <dt className="text-lg font-medium text-blue-500 leading-6 ">
              Enroll this student to current term?
            </dt>
            {}
            <dd className="flex items-end  mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">
              <button
                disabled={currentTerm?.name != publishedTerm?.name}
                onClick={() => {
                  if (params.id) {
                    dispatch(
                      setOpenModal({
                        isOpen: true,
                        type: "enrollStudentToCurrentTerm",
                        data: {
                ...
                className="px-4 py-2 rounded bg-indigo-600 text-white cursor-pointer disabled:bg-slate-200 disabled:cursor-not-allowed"
              >
                Enroll Student to current term
              </button>
              {publishedTerm?.name == currentTerm?.name && (
                <span className="text-red-400 text-xs font-medium">
                  *You are seeing this option because you have current term &
                  published term to be same.
                </span>
              )}
            </dd>
      ...
      <EnrollStudentToCurrentTermToCurrentModal />
    </div>
  )
}
